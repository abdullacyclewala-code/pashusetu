...
| ✅ Image model v1 | Done | `pending` | Report photo step runs a real 4.8 MB ONNX model locally in the browser and presents normal-appearing, LSD-like, or inconclusive results plus a broad normal/abnormal screen. It never changes the symptom triage. | Trained on Mendeley `10.17632/w36hpf86j2.1` (CC BY 4.0). pHash grouping quarantined 13 cross-label near-duplicates and prevented duplicate groups crossing train/validation/test. Held-out test: 203 images, accuracy 0.847, balanced accuracy 0.763, ROC AUC 0.963; LSD-like precision 0.944, recall 0.540. High-confidence threshold 0.89; 0.20–0.89 is deliberately inconclusive. Model card: `ml/MODEL_CARD.md`; metrics: `ml/model_metrics.json`; reproducible pipeline: `ml/train_lsd_screen.py`; runtime: `lib/image-model/infer.ts`. Other diseases are explicitly unsupported until defensibly licensed and labelled data is audited. |
