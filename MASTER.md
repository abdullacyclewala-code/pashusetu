...
| ✅ Mobile location + navigation cleanup | Done | `pending` | “Use current location” now reverse-geocodes the actual GPS coordinate through an authenticated server route instead of blindly choosing the nearest seeded village. Registered-village fallback is accepted only within 10 km; otherwise the app keeps the true GPS point, clears the misleading profile location, and asks for locality/district. Verified Crawford Market coordinates resolve to Fort / Mumbai City District rather than Panvel. Mobile navigation is capped at five destinations: role-priority tabs, the central Report action where applicable, and a More sheet for secondary tools; desktop navigation remains complete. |
| ✅ Image model v2 + combined pipeline | Done | `pending` | Report photo step runs a real 4.8 MB three-way ONNX model locally in the browser and presents normal-appearing, LSD-like, FMD-like, or inconclusive results plus a broad normal/abnormal screen. The structured result travels in the offline queue and migration `0015_image_model_pipeline.sql` (applied live 5 Sep 2026) stores it in its own `triage_results` row (`source='image_model'`) through an ownership-validating RPC. A combined summary explains agreement, disagreement, normal-image and inconclusive cases; ONNX Runtime's required threaded WASM loader (`ort-wasm-simd-threaded.mjs`) is shipped beside the WASM binary (fixes the initial silent browser inference failure); the broader rule engine always controls urgency and is never overwritten or downgraded. Full training/testing report: `ml/TRAINING_AND_TESTING_REPORT.md`. | Trained on 3,240 usable images from Kaggle `devang03mgr/cattle-diseases-datasets` (dataset page: ODC Database Contents License 1.0); 4 images in 2 conflicting pHash groups quarantined and duplicate groups kept inside one split. Held-out test: 648 images, accuracy 0.855, balanced accuracy 0.862, macro OVR AUC 0.962; class F1: FMD 0.854, healthy 0.836, LSD 0.875. Product gate requires ≥0.75 probability and ≥0.20 margin, otherwise inconclusive. `MODEL_CARD_V2.md` explicitly documents weak original provenance, heterogeneous web images, illustrations/watermarks, possible uncaught augmentations, and that these are dataset-label metrics—not clinical validation. Other diseases remain unsupported. |
