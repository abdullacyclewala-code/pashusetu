-- P10 judge-demo timeline. Idempotent; uses the test farmer and realistic Shirur locations.
...
begin
 ...
 for i in 1..5 loop
  ...
  lat:=18.8277+(i-3)*.004; lng:=74.3743+(i-3)*.005;
  ...
  values(rid,farmer,'cattle','["fever","mouth_blisters","drooling","lameness","milk_drop"]',
   ...
   st_setsrid(st_makepoint(lng,lat),4326)::geography,'Shirur','Shirur','Pune','triaged',ts,ts,'app')
 ...
 perform detect_clusters(p_district=>'Pune');
 perform run_early_warning('2026-09-04','Pune');
end $$;
